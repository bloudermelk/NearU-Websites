#!/usr/bin/env node
/**
 * Assembles <site>/app/theme.css (and mirrors fonts into <site>/public/fonts/)
 * from the ORIGINAL (un-optimized) stylesheets of a brand's live WordPress
 * site, in the exact order WordPress enqueues them.
 *
 * Usage (from inside a brand site folder): npm run content:theme-css
 *   Reads content/site.json `source.origin` for the live site URL and
 *   content/pages.txt for the page list to union block styles across.
 *
...
import { CONTENT_DIR, SITE_DIR, loadSiteJson, resolveSiteSlug } from "./lib/env.mjs";
...
const BRAND_DIR = CONTENT_DIR;
const OUT = join(SITE_DIR, "app", "theme.css");
const FONTS_DIR = join(SITE_DIR, "public", "fonts");
const UA =
...
/** Every font file the theme CSS references, so we can mirror them into content/fonts/. */
