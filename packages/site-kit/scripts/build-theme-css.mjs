...
/**
 * Also emits app/theme-css.ts — the same CSS as a string module. The site's
 * layout inlines it into a <style> in <head> (via the kit's RootLayout
 * `themeCss` prop) instead of importing theme.css as a stylesheet. Measured
 * reason: with the CSS as a separate <link>, first paint waited on a second
 * request that shared a slow link with the hero image and async JS (1.5–2.5s
 * later than the live WordPress site, which inlines its critical CSS). Inline
 * in the layout it rides in the entry HTML only — not in per-page navigation
 * payloads (unlike Next's experimental.inlineCss, which multiplied it 3–4x).
 * theme.css stays as the readable source of truth; both files are committed.
 */
export function writeThemeModule(css) {
  const modulePath = join(SITE_DIR, "app", "theme-css.ts");
  const shipped = prepareCssForShipping(css);
  const body =
    `// GENERATED by \`npm run content:theme-css\` from ./theme.css — do not edit.\n` +
    `// Inlined into <head> by the layout; see packages/site-kit/src/routes/layout.tsx.\n` +
    `// Comments and hot-linked WordPress plugin assets are stripped (see prepareCssForShipping).\n` +
    `export const themeCss = ${JSON.stringify(shipped)};\n`;
  writeFileSync(modulePath, body, "utf8");
  console.log(`Wrote ${modulePath} (${Math.round(shipped.length / 1024)}KB CSS shipped, ${Math.round(css.length / 1024)}KB in theme.css).`);
}

/**
 * theme.css keeps the human-readable provenance comments (`/* ===== /wp-content/… ===== *\/`,
 * sourceURL markers). What we SHIP is stripped of them: they're dead bytes in
 * every page, and — because the CSS is inlined into the HTML — they were the
 * loudest "this is WordPress" fingerprint for tech-stack detectors (50
 * `/wp-content/` + `/wp-includes/` mentions per page, all in comments). Also
 * neutralises the couple of `url(https://<live-site>/wp-content/…)` hot-links
 * left in third-party plugin CSS (a forms-plugin spinner we never render).
 */
export function prepareCssForShipping(css) {
  return css
    .replace(/\/\*[\s\S]*?\*\//g, "")
    .replace(/url\((['"]?)https?:\/\/[^)'"]*\/wp-(?:content|includes)\/[^)'"]*\1\)/gi, "none")
    .replace(/\n{2,}/g, "\n")
    .trim();
}
