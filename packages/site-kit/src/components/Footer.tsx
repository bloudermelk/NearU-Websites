...
export async function Footer() {
  const { business, footer, company } = await getSite();
    ...
    <footer id="colophon" className="site-footer | container">
      ...
      <div className="site-footer-copyright">
        ...
        <div className="site-footer-copyright__p">
          ...
          {/* Legal links are company-wide (identical on every NearU brand) — from the `companies` row. */}
          {company.legalLinks.map((l) => (
            <Link key={l.href} className="inline-footer-menu-item" href={l.href}>
              {l.label}
            </Link>
          ))}{" "}
          {licenseLines.length > 0 && (
