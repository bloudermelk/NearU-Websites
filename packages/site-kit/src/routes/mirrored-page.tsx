 ...
 * routes/home.tsx.
 *
 * Each site's `app/[...slug]/page.tsx` re-exports these and declares the
 * segment config itself (Next.js requires `dynamicParams`/`revalidate` to be
 * literals in the route file, so they can't live here):
 *   export const dynamicParams = true;
 *   export const revalidate = 3600;
 * That means pages known at build time are pre-rendered, but new pages added
 * in Supabase after deploy (or edits to existing ones) become visible without
 * a redeploy — Next.js regenerates in the background at most once per window.
