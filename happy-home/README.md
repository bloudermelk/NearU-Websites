...
**happyhomeheatingandcooling.com** — a NearU brand website. This folder is a complete, independently
deployable Next.js app; it is one Vercel project (Root Directory: `happy-home`).
...
| `next.config.ts` | One line: `createSiteConfig({ siteSlug: "happy-home" })` |
...
(`SITE_SLUG=happy-home`) is baked in by `next.config.ts`.
